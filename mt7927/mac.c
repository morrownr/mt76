...
static void
mt7927_mac_wcid_add_poll(struct mt792x_dev *dev, struct mt76_wcid *wcid)
{
	struct mt76_dev *mdev = &dev->mt76;

	if (test_bit(MT76_MCU_RESET, &mdev->phy.state) || !wcid || !wcid->sta ||
	    wcid->idx >= MT792x_WTBL_SIZE)
		return;

	mt76_wcid_add_poll(mdev, wcid);
}
...
static void mt7927_mac_sta_poll(struct mt792x_dev *dev)
{
	...
	while (true) {
		...
		if (idx >= MT792x_WTBL_SIZE ||
		    rcu_access_pointer(dev->mt76.wcid[idx]) != &mlink->wcid ||
		    !mlink->wcid.sta || !msta)
			continue;
...
mt7927_mac_fill_rx(struct mt792x_dev *dev, struct sk_buff *skb)
{
	...
	if (status->wcid) {
		...
		mt7927_mac_wcid_add_poll(dev, &mlink->wcid);
	}
...
/* compat: see note in mt76_connac_mac.c -- 7.1 mac80211 action API change */
	...
	/* Ensure injected management frames are recognized correctly */
	if (info->flags & IEEE80211_TX_CTL_INJECTED &&
	    fc_type == IEEE80211_FTYPE_MGMT >> 2)
		fc_stype = (le16_to_cpu(fc) & IEEE80211_FCTL_STYPE) >> 4;
	else
		fc_stype = (le16_to_cpu(fc) & IEEE80211_FCTL_STYPE) >> 4;
