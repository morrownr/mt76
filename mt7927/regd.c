...
void mt7927_regd_be_ctrl(struct mt792x_dev *dev, u8 *alpha2)
{
	...
	struct mt7927_clc_rule_v2 *rule;
	struct mt7927_clc *clc;
	...
	clc = (struct mt7927_clc *)phy->clc[MT792x_CLC_BE_CTRL];
	pos = clc->data;

	while (1) {
		rule = (struct mt7927_clc_rule_v2 *)pos;
...
static void
mt7927_regd_channel_update(struct wiphy *wiphy, struct mt792x_dev *dev)
{
#define IS_UNII_INVALID(idx, sfreq, efreq, cfreq) \
	(!(dev->phy.clc_chan_conf & BIT(idx)) && (cfreq) >= (sfreq) && (cfreq) <= (efreq))
#define MT7927_UNII_59G_IS_VALID	0x1
#define MT7927_UNII_6G_IS_VALID	0x1e
	...
	if (mtcl_conf != MT792X_ACPI_MTCL_INVALID) {
		if ((mtcl_conf & 0x3) == 0)
			dev->phy.clc_chan_conf &= ~MT7927_UNII_59G_IS_VALID;
		if (((mtcl_conf >> 2) & 0x3) == 0)
			dev->phy.clc_chan_conf &= ~MT7927_UNII_6G_IS_VALID;
	}
			   ...
			   enum environment_cap country_ie_env)
{
	...
	ret = mt7927_mcu_set_clc(dev, alpha2, country_ie_env);
	if (ret < 0)
		goto err;

	mt7927_regd_be_ctrl(dev, alpha2);
	mt7927_regd_channel_update(wiphy, dev);

	ret = mt7927_mcu_set_channel_domain(hw->priv);
	if (ret < 0)
		goto err;
...
EXPORT_SYMBOL_GPL(mt7927_mcu_regd_update);
...
void mt7927_regd_notifier(struct wiphy *wiphy, struct regulatory_request *req)
{
	...
	mt7927_mcu_regd_update(dev, req->alpha2,
			       req->country_ie_env);
	return;
}

static bool
mt7927_regd_is_valid_alpha2(const char *alpha2)
{
	if (!alpha2)
		return false;
...
int mt7927_regd_change(struct mt792x_phy *phy, char *alpha2)
{
	...
	if (!mt7927_regd_is_valid_alpha2(alpha2) ||
	    !mt7927_regd_clc_supported(dev) ||
	    dev->regd_user)
		return -EINVAL;
	...
	if (phy->chip_cap & MT792x_CHIP_CAP_11D_EN) {
		...
		return mt7927_mcu_set_clc(dev, alpha2, ENVIRON_INDOOR);
	}
}
EXPORT_SYMBOL_GPL(mt7927_regd_change);
